...
export function getReports(queryParams: ParsedQs): Promise<IReport[] | IReportDetails[]> {
    ...
    const reportParams: Partial<IReport> = allowedParams.reduce((params: { [key: string]: number }, key: string): { [key: string]: number } => {
        if (queryParams[key]) {
...
export function getReport(reportId: string): Promise<IReport | null> {
    return getReportByIdModel(parseInt(reportId))
